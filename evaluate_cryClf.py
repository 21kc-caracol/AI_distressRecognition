...
class global_For_Clf():
    def __init__(self):
        ...
        self.data_file_path = 'csv/cry/data_cry_mfcc_20.csv'  # cry
        self.clf_label = 'cry'  # cry
        self.csv_to_pkl_path = "pickle/scream/combined_lower_amount.pkl" # relevant to modular file TODO currently this is only for scream
        self.path_csv_train_test_data = "csv/cry/train_test_data.csv"  # chosen 1:1 ratio data, selected from data.csv
        self.resultsPath = 'results/cry/experiments_results.csv'
...
def create_lower_bound_data_panda(csv_path, label):
    ...
    nearMissMaxAmount = lower_amount // screamGlobals.nearMissRatio
...
def get_Repeated_strtfy_results(combined_lower_amount, k, repetitions):
    ...
    clf_labels_amount = len(data_no_fileName.loc[data_no_fileName['label'] == screamGlobals.clf_label])
    nearMiss_clf_amount = len(data_no_fileName.loc[data_no_fileName['label'] == screamGlobals.nearMissLabel])
    ...
    clf_NearMiss_label_int = encoder.transform([screamGlobals.nearMissLabel])[0]
...
def experiment_data_size_with_model():
    ...
    if screamGlobals.resultsPath == 'results/cry/experiment3.csv':
...
def experiment1():
    ...
    date = get_date()
    screamGlobals.userInput = f'experiment_1_date_{date}'
    screamGlobals.data_file_path = 'csv/cry/data_experiment_1.csv'
...
def experiment2():
    ...
    # set global variables for experiment
    date = get_date()
    screamGlobals.userInput = f'experiment_2_date_{date}'
    ...
    for size in tqdm([1, 5, 10, 12, 15, 20, 25, 30, 35, 40]):
        ...
        for model in tqdm(models_to_check):
            ...
            mfcc_amount = size
            screamGlobals.data_file_path = f'csv/cry/data_experiment_2_mfcc_{mfcc_amount}.csv'
...
def results_to_csv_experiment3(csv_results_head, csv_results):
    ...
    csv_data_results = f'{screamGlobals.clf_label} {screamGlobals.get_total_samples()}' \
                       f' {screamGlobals.positives} {screamGlobals.negatives}' \
                       f' {screamGlobals.nearMissRatio} {screamGlobals.nearMiss_samples}' \
                       f' {screamGlobals.getInputDim()} {screamGlobals.Kfold_testSize} {screamGlobals.k_folds}' \
                       f' {screamGlobals.k_fold_repetitions} {screamGlobals.sampling_data_repetitions}'
...
def experiment3():
    ...
    # set global variables for experiment
    date = get_date()
    screamGlobals.userInput = f'experiment_3_date_{date}'
    screamGlobals.resultsPath = 'results/cry/experiment3.csv'
    ...
    for ratio in tqdm(nearMiss_ratios):
        screamGlobals.nearMissRatio = ratio
...
def experiment5():
    ...
    # set global variables for experiment
    date = get_date()
    screamGlobals.userInput = f'experiment_5_date_{date}'
    screamGlobals.n_mfcc = 20  # verifying correct value for experiment
    ...
    for model in tqdm(models_to_check):
        ...
        screamGlobals.data_file_path = f'csv/cry/data_experiment_2_mfcc_20.csv'

        experiment_data_size_with_model_different_mfcc()

    # now check best model with 15 as mfcc resolution
    screamGlobals.n_mfcc = 15
    screamGlobals.data_file_path = f'csv/cry/data_experiment_2_mfcc_15.csv'

    screamGlobals.model = load_bestModel()
    experiment_data_size_with_model()
